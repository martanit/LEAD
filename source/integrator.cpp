...
void Integrator::langevin_verlet()
{
  ...
  for(unsigned int i=0; i<(*m_poly).get_poly_sphere(); ++i){
    ...
    (*m_poly_new).set_vx(m_conf.pbc((*m_poly_new).get_x(i)-(*m_poly_old).get_x(i))/(2.*m_integrator_timestep), i);
    
    (*m_poly_new).set_vy(m_conf.pbc((*m_poly_new).get_y(i)-(*m_poly_old).get_y(i))/(2.*m_integrator_timestep), i);
    
    (*m_poly_new).set_vz(m_conf.pbc((*m_poly_new).get_z(i)-(*m_poly_old).get_z(i))/(2.*m_integrator_timestep), i);
...
void Integrator::langevin_overdamped()
{
  ...
  for(unsigned int i=0; i<(*m_poly).get_poly_sphere(); ++i){
    ...
    (*m_poly).set_vx(m_conf.pbc((*m_poly_new).get_x(i)-(*m_poly).get_x(i))/m_integrator_timestep, i); 
    
    (*m_poly).set_vy(m_conf.pbc((*m_poly_new).get_y(i)-(*m_poly).get_y(i))/m_integrator_timestep, i); 
    
    (*m_poly).set_vz(m_conf.pbc((*m_poly_new).get_z(i)-(*m_poly).get_z(i))/m_integrator_timestep, i); 
