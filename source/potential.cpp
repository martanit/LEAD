    ...
    for (auto && k : sphere[i]){
       ...
       if(dr < m_pot_rcut*m_pot_rcut){
           ...
           if(attractive){
               ...
               if(compute_energy) m_poly.add_energy(-2.*m_pot_epsilon*
                                                    m_pot_rmin_6/std::pow(dr,3));
           }
          
           m_poly.add_force(i, f_x, f_y, f_z);
           m_poly.add_force(k, -f_x, -f_y, -f_z);
           
           if(compute_energy) m_poly.add_energy(m_pot_epsilon*
                                                m_pot_rmin_12/std::pow(dr,6));
       }
     }
  }
}
...
{ 
  for (int i=0; i<m_poly.get_poly_sphere()-1; ++i ){
      spring_x = -k * ( m_poly.dist(i, i+1) - m_pot_rmin) * 
                      ( m_poly.get_x(i) - m_poly.get_x(i+1))
                      /m_poly.dist(i, i+1);
      spring_y = -k * ( m_poly.dist(i, i+1) - m_pot_rmin) * 
                      ( m_poly.get_y(i) - m_poly.get_y(i+1))
                      /m_poly.dist(i, i+1);
      spring_z = -k * ( m_poly.dist(i, i+1) - m_pot_rmin) * 
                      ( m_poly.get_z(i) - m_poly.get_z(i+1))
                      /m_poly.dist(i, i+1);
...
{  
  //iterate over each extruder
     for(auto &i: m_vector_extr) {
        spring_x = -k_extr * ( m_poly.dist((*i).get_l(), (*i).get_r()) - extr_lenght) * 
                      ( m_poly.get_x((*i).get_l()) - m_poly.get_x((*i).get_r()))
                      /m_poly.dist((*i).get_l(), (*i).get_r());
        spring_y = -k_extr * ( m_poly.dist((*i).get_l(), (*i).get_r()) - extr_lenght) * 
                      ( m_poly.get_y((*i).get_l()) - m_poly.get_y((*i).get_r()))
                      /m_poly.dist((*i).get_l(), (*i).get_r());
        spring_z = -k_extr * ( m_poly.dist((*i).get_l(), (*i).get_r()) - extr_lenght) * 
