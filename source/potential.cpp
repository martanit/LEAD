...
void Potential::box(bool compute_energy) {
    for (unsigned int i = 0; i < m_poly.get_poly_nmonomers(); ++i) {
        ...
        if(m_poly.get_x(i) < m_poly.get_xcm()-m_pot_box_length/2.){
            f_x += -m_pot_kside *(m_poly.get_x(i)-m_poly.get_xcm()+m_pot_box_length/2.);
            if(compute_energy)
                m_poly.add_energy(m_pot_kside/2.*std::pow(m_poly.get_x(i)-m_poly.get_xcm()+m_pot_box_length/2., 2));
        }
        if(m_poly.get_y(i) < m_poly.get_ycm()-m_pot_box_length/2.) {
            f_y += -m_pot_kside *(m_poly.get_y(i)-m_poly.get_ycm()+m_pot_box_length/2.);
            if(compute_energy)
                m_poly.add_energy(m_pot_kside/2.*std::pow(m_poly.get_y(i)-m_poly.get_ycm()+m_pot_box_length/2., 2));
        }
        if(m_poly.get_z(i) < m_poly.get_zcm()-m_pot_box_length/2.) {
            f_z += -m_pot_kside *(m_poly.get_z(i)-m_poly.get_zcm()+m_pot_box_length/2.);
            if(compute_energy)
                m_poly.add_energy(m_pot_kside/2.*std::pow(m_poly.get_z(i)-m_poly.get_zcm()+m_pot_box_length/2., 2));
        }
        if(m_poly.get_x(i) > m_poly.get_xcm()+m_pot_box_length/2.) {
            f_x += -m_pot_kside *(m_poly.get_x(i)-m_poly.get_xcm()-m_pot_box_length/2.);
            if(compute_energy)
