...
#ifndef EXTRUDER_FIELD_H_
#define EXTRUDER_FIELD_H_
...
public:
  ...
  ExtruderField(Parameters);

  ~ExtruderField(){};
  
  void operator=(const ExtruderField &rhs) {
  ...
  //access funciton
  const double &get_delta_c() const { return m_delta_c; }
  const double &get_c(int i, int j, int k) 
	  const { return m_extruder_c[i][j][k]; }
  const double &get_field_step() const { return m_field_step; }
  const double &get_field_length() const { return m_field_length; }
